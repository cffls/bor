...
func (s *StateDB) ApplyMVWriteSet(writes []blockstm.WriteDescriptor) {
	...
	for i := range writes {
		...
		if path.IsState() {
			...
			case BalancePath:
				// Always use absolute value from the worker stateObject.
				// The worker's balance is correct: base + prior_deltas + this_tx_ops.
				// Delta-based settlement (via GetTxDelta) is unreliable due to
				// speculative timing differences in accumulated deltas.
				if pair.src != nil {
