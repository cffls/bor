...
func (task *ExecutionTask) Execute(mvh *blockstm.MVHashMap, incarnation int) (err error) {
	...
	defer func() {
		if r := recover(); r != nil {
			...
			err = blockstm.ErrExecAbort{DependencyIndex: task.statedb.DepTxIndex()}
	...
	if *task.shouldDelayFeeCal {
		...
		if _, ok := task.statedb.MVReadMap()[blockstm.NewAddressKey(blockContext.Coinbase)]; ok {
			task.shouldRerunWithoutFeeDelay = true
		}
		if _, ok := task.statedb.MVReadMap()[blockstm.NewAddressKey(task.result.BurntContractAddress)]; ok {
			task.shouldRerunWithoutFeeDelay = true
		}
	} else {
