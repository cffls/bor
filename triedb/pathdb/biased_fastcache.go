...
func (c *AddressBiasedCache) preloadAddressAsync(db ethdb.Database, addr common.Address, cacheSize int) {
	...
	for len(queue) > 0 {
		...
		// Both Has and Set are thread-safe on fastcache (internal sharding).
		// A narrow race between Has and Set is acceptable: this is a cache,
		// not the source of truth, so a stale write only causes a future
		// cache miss that self-corrects on the next disk read.
